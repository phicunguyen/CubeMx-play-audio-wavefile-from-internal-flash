...
void AudioPlay_Start (uint32_t AudioFreq) {
    cnt = 0;
    uint32_t audIndex=44;
    
    AudioTotalSize = waveformat-> FileSize;  
    /* Get Data from USB Flash Disk */
    WaveDataLength = waveformat-> FileSize;

    memcpy(&Audio_Buffer[0], &audio_data[44], AUDIO_BUFFER_SIZE);
    audIndex += AUDIO_BUFFER_SIZE;
    AudioRemSize = WaveDataLength - audIndex;
    ...
    while (AudioRemSize != 0) {
        if (buffer_offset == BUFFER_OFFSET_HALF) {
            memcpy(&Audio_Buffer[0], &audio_data[audIndex], AUDIO_BUFFER_SIZE/2);
            buffer_offset = BUFFER_OFFSET_NONE;
            AudioRemSize -= (AUDIO_BUFFER_SIZE / 2);
            audIndex+=(AUDIO_BUFFER_SIZE / 2);
            ...
            //printf ((char *) str3, "% 02d:% 02d", (int) (dur / 60), (int) (dur% 60));
        }

        if (buffer_offset == BUFFER_OFFSET_FULL) {
            memcpy(&Audio_Buffer[AUDIO_BUFFER_SIZE / 2], &audio_data[audIndex], AUDIO_BUFFER_SIZE/2);
            AudioRemSize -= (AUDIO_BUFFER_SIZE / 2);
            buffer_offset = BUFFER_OFFSET_NONE;
            audIndex+=(AUDIO_BUFFER_SIZE / 2);
        }
